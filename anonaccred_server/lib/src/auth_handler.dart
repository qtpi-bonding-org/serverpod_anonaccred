...
class AnonAccredAuthHandler {
  ...
  static Future<AuthenticationInfo?> handleAuthentication(
    ...
    session.log(
      'AnonAccredAuthHandler: handleAuthentication called',
      level: LogLevel.info,
    );
    session.log(
      'AnonAccredAuthHandler: token length: ${token.length}',
      level: LogLevel.info,
    );
    session.log(
      'AnonAccredAuthHandler: token prefix: ${token.length > 20 ? token.substring(0, 20) : token}...',
      level: LogLevel.info,
    );
    ...
    try {
      ...
      session.log(
        'AnonAccredAuthHandler: trimmedToken length: ${trimmedToken.length}',
        level: LogLevel.info,
      );
      ...
      if (trimmedToken.startsWith('Bearer ')) {
        ...
        session.log(
          'AnonAccredAuthHandler: Extracted public key from Bearer token, length: ${devicePubKey.length}',
          level: LogLevel.info,
        );
        session.log(
          'AnonAccredAuthHandler: Device public key prefix: ${devicePubKey.length > 20 ? devicePubKey.substring(0, 20) : devicePubKey}...',
          level: LogLevel.info,
        );
        ...
        try {
          ...
          session.log(
            'AnonAccredAuthHandler: Decoded Basic token: ${decoded.length > 20 ? decoded.substring(0, 20) : decoded}...',
            level: LogLevel.info,
          );
          ...
          session.log(
            'AnonAccredAuthHandler: Failed to decode Basic token: $e',
        ...
        session.log(
          'AnonAccredAuthHandler: Using raw public key from token, length: ${devicePubKey.length}',
          level: LogLevel.info,
        );
      }
      ...
      if (devicePubKey == null || devicePubKey.isEmpty) {
        session.log(
          'AnonAccredAuthHandler: ERROR - Could not extract device public key from token',
          level: LogLevel.error,
        );
        return null;
      }
      ...
      session.log(
        'AnonAccredAuthHandler: Final device public key length: ${devicePubKey.length}',
        level: LogLevel.info,
      );

      // Validate device public key format
      session.log(
        'AnonAccredAuthHandler: Validating device public key format...',
        level: LogLevel.info,
      );
      AnonAccredHelpers.validatePublicKey(devicePubKey, 'authentication');
      session.log(
        'AnonAccredAuthHandler: Device public key format is valid',
        level: LogLevel.info,
      );

      // Verify device exists and is active
      session.log(
        'AnonAccredAuthHandler: Looking up device in database...',
        level: LogLevel.info,
      );
      ...
      if (device == null) {
        session.log(
          'AnonAccredAuthHandler: ERROR - Device not found in database',
          level: LogLevel.error,
        );
      } else {
        session.log(
          'AnonAccredAuthHandler: Device found, ID: ${device.id}, accountId: ${device.accountId}, isRevoked: ${device.isRevoked}',
          level: LogLevel.info,
        );
      }
      ...
      session.log(
        'AnonAccredAuthHandler: Device is active, creating AuthenticationInfo...',
        level: LogLevel.info,
      );
      ...
      session.log(
        'AnonAccredAuthHandler: Authentication successful, authId: "${authInfo.authId}"',
        level: LogLevel.info,
      );
      return authInfo;
      ...
      session.log(
        'AnonAccredAuthHandler: Authentication exception: ${e.toString()}',
        level: LogLevel.error,
      );
      return null;
    } on Exception catch (e) {
      session.log(
        'AnonAccredAuthHandler: Authentication error: ${e.toString()}',
        level: LogLevel.error,
      );
      return null;
    }
