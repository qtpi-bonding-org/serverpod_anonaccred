  ...
  group('X402 Integration Compatibility Property Tests', () {
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          if (paymentResult.success) {
            expect(paymentResult.transactionHash, isNotNull);
            expect(paymentResult.transactionHash, startsWith('x402_tx_'));
            expect(paymentResult.errorMessage, isNull);
          } else {
            expect(paymentResult.transactionHash, isNull);
    ...
    test(
      ...
      () async {
        ...
        for (final railType in [PaymentRail.x402_http, PaymentRail.monero, PaymentRail.apple_iap]) {
    ...
    test(
      ...
      () async {
        ...
        final failureResult = await x402Rail.processCallback(failureCallbackData);
        expect(failureResult.success, isFalse);
        expect(failureResult.orderId, equals('test_order_123'));
        expect(failureResult.transactionHash, isNull);
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          expect(recreatedRequest.paymentRef, equals(originalRequest.paymentRef));
          ...
          expect(recreatedRequest.railData['destinationAddress'], isA<String>());
        }
      },
    );
