...
void main() {
  withServerpod('Inventory Manager Property Tests', (
    sessionBuilder,
    endpoints,
  ) {
    final random = Random();
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          final initialRecord = initialInventory
              .where((inv) => inv.consumableType == consumableType)
              .firstOrNull;
          ...
          expect(
            newRecord.lastUpdated.isAfter(beforeAddition) ||
                newRecord.lastUpdated.isAtSameMomentAs(beforeAddition),
            isTrue,
          );
          expect(
            newRecord.lastUpdated.isBefore(afterAddition) ||
                newRecord.lastUpdated.isAtSameMomentAs(afterAddition),
            isTrue,
          );
          ...
          final finalRecord = finalInventory
              .where((inv) => inv.consumableType == consumableType)
              .first;

          expect(
            finalRecord.lastUpdated.isAfter(beforeSecondAddition) ||
                finalRecord.lastUpdated.isAtSameMomentAs(beforeSecondAddition),
            isTrue,
          );
          expect(
            finalRecord.lastUpdated.isBefore(afterSecondAddition) ||
                finalRecord.lastUpdated.isAtSameMomentAs(afterSecondAddition),
            isTrue,
          );
          ...
          final firstRecord = multiInventory
              .where((inv) => inv.consumableType == consumableType)
              .first;
          final secondRecord = multiInventory
              .where((inv) => inv.consumableType == secondConsumableType)
              .first;
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          final emptyInventory = await InventoryManager.getInventory(
            session,
            accountId,
          );
          ...
          final fullInventory = await InventoryManager.getInventory(
            session,
            accountId,
          );
          ...
          for (int j = 0; j < numItems; j++) {
            ...
            final inventoryRecord = fullInventory
                .where((inv) => inv.consumableType == expectedType)
                .first;
          ...
          final inventoryBeforeQueries = await InventoryManager.getInventory(
            session,
            accountId,
          );
          ...
          final inventoryAfterQueries = await InventoryManager.getInventory(
            session,
            accountId,
          );
          expect(
            inventoryAfterQueries.length,
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          final stillEmptyInventory = await InventoryManager.getInventory(
            session,
            accountId,
          );
