...
void main() {
  group('Cryptographic Security Property Tests', () {
    ...
    test('Property 5: Cryptographic data isolation - For any account or device creation, the system should store only public keys and encrypted data keys, never private keys or unencrypted data', () {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        expect(account.publicMasterKey.length, equals(64)); // Ed25519 public key format
        ...
        // Verify no private key storage capability
        expect(device.publicSubKey.length, equals(64)); // Ed25519 public key format
    ...
    test('Property 6: Encryption separation - For any account with multiple devices, the system should maintain separate encrypted data keys for master account access and individual device access', () {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        for (final device in devices) {
          expect(device.encryptedDataKey, isNot(equals(account.encryptedDataKey)));
        ...
        for (int j = 0; j < devices.length; j++) {
          for (int k = j + 1; k < devices.length; k++) {
            expect(devices[j].encryptedDataKey, isNot(equals(devices[k].encryptedDataKey)));
            expect(devices[j].publicSubKey, isNot(equals(devices[k].publicSubKey)));
          }
        }
    ...
    test('Property 20: Signature verification support - For any valid Ed25519 signature and public key pair, the system should correctly validate the signature', () async {
      // Run 5 iterations during development
      for (int i = 0; i < 5; i++) {
        ...
        expect(() async => await CryptoUtils.verifyEd25519Signature(
          message: message,
    ...
    test('Property 21: Cryptographic error security - For any cryptographic operation failure, error information should be provided without exposing key material', () async {
      // Run 5 iterations during development
      for (int i = 0; i < 5; i++) {
        ...
        for (final invalidKey in invalidPublicKeys) {
          try {
            ...
            expect(authException.code, equals(AnonAccredErrorCodes.cryptoInvalidPublicKey));
            expect(authException.message, contains('Invalid Ed25519 public key format'));
            ...
            if (invalidKey.isNotEmpty) {
              ...
              expect(authException.details?.values.any((v) => v.contains(invalidKey)), isFalse);
            }
          }
        }
        ...
        for (final invalidSignature in invalidSignatures) {
          try {
            ...
            expect(authException.code, equals(AnonAccredErrorCodes.cryptoInvalidSignature));
            expect(authException.message, contains('Invalid Ed25519 signature format'));
            expect(authException.operation, equals('verifyEd25519Signature'));
            ...
            if (invalidSignature.isNotEmpty) {
              expect(authException.message, isNot(contains(invalidSignature)));
              expect(authException.details?.values.any((v) => v.contains(invalidSignature)), isFalse);
            }
          }
        }
        ...
        try {
          ...
          expect(authException.code, equals(AnonAccredErrorCodes.cryptoInvalidMessage));
        ...
        try {
          ...
          expect(authException.code, equals(AnonAccredErrorCodes.cryptoFormatError));
          expect(authException.message, anyOf([
          ...
          if (authException.details != null && authException.details!.isNotEmpty) {
            // If details exist, verify they don't expose the invalid input
            expect(authException.details!.values.any((v) => v.contains('invalid_hex_g')), isFalse);
          }
        }
      }
    });
