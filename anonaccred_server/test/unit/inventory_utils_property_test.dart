...
void main() {
  withServerpod('Inventory Utils Property Tests', (sessionBuilder, endpoints) {
    ...
    test('Property 9: Consumption Utility Success - For any consumption request with sufficient balance, the utility should atomically decrement the balance and return success', () async {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        final consumeQuantity = initialQuantity * (0.1 + (random.nextDouble() * 0.8)); // 10-90% of initial
        ...
        expect(result.availableBalance, closeTo(initialQuantity - consumeQuantity, 0.0001));
        ...
        expect(newBalance, closeTo(initialQuantity - consumeQuantity, 0.0001));
        
        // Test multiple consumptions from same account
        final secondConsumeQuantity = newBalance * 0.5; // Consume half of remaining
        ...
        expect(secondResult.availableBalance, closeTo(newBalance - secondConsumeQuantity, 0.0001));
        ...
        expect(finalBalance, closeTo(newBalance - secondConsumeQuantity, 0.0001));
    ...
    test('Property 9 Extension: Consumption with multiple consumable types', () async {
      // Create a real account for this test
      final publicKey = _generateRandomPublicKey();
    ...
    test('Property 9 Extension: Fractional consumption', () async {
      ...
      expect(result.success, isTrue);
      expect(result.errorMessage, isNull);
      expect(result.availableBalance, closeTo(initialQuantity - fractionalConsume, 0.0001));
    ...
    test('Consumption failure scenarios - insufficient balance and invalid inputs', () async {
      // Create a real account for this test
      final publicKey = _generateRandomPublicKey();
      ...
      expect(insufficientResult.errorMessage, contains('Insufficient balance'));
      ...
      expect(negativeResult.errorMessage, contains('Quantity must be positive'));
      ...
      expect(nonExistentResult.errorMessage, contains('Insufficient balance'));
    ...
    test('Concurrent consumption operations (atomicity test)', () async {
      ...
      for (int i = 0; i < numConcurrentOperations; i++) {
        futures.add(InventoryUtils.tryConsume(
          session,
          accountId: accountId,
          consumableType: consumableType,
          quantity: consumeQuantity,
      ...
      final expectedBalance = initialQuantity - (successfulOperations * consumeQuantity);
