...
void main() {
  withServerpod('Device Revocation Property Tests', (sessionBuilder, endpoints) {
    final random = Random();

    test('Property 6: Revocation enforcement - For any device that has been revoked, all subsequent authentication attempts should fail with a revoked device error', () async {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        if (!authResultBefore.success) {
          expect(authResultBefore.errorCode, isNot(equals('AUTH_DEVICE_REVOKED')));
        }
        ...
        final newChallenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
        final newSignature = _generateRandomEd25519Signature();
        
        final authResultNewChallenge = await endpoints.device.authenticateDevice(
          sessionBuilder,
          devicePublicSubKey,
          newChallenge,
          newSignature,
        ...
        expect(authResultNewChallenge.errorCode, equals('AUTH_DEVICE_REVOKED'));

        // Verify revocation is persistent - multiple authentication attempts should all fail
        for (int j = 0; j < 3; j++) {
          final persistentChallenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
          final persistentSignature = _generateRandomEd25519Signature();
          
          final persistentAuthResult = await endpoints.device.authenticateDevice(
            sessionBuilder,
            devicePublicSubKey,
            persistentChallenge,
            persistentSignature,
          ...
          expect(persistentAuthResult.errorCode, equals('AUTH_DEVICE_REVOKED'));
        }
      }
    });
  });
