...
void main() {
  withServerpod('Order Manager Property Tests', (sessionBuilder, endpoints) {
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          for (final consumable in consumables) {
            ...
            expect(
              consumable.quantity,
              equals(items[consumable.consumableType]),
            );
          }
          ...
          expect(
            secondTransaction.externalId,
            isNot(equals(transaction.externalId)),
          );
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          invalidItems[validItems.keys.first] =
              -1.0; // Invalid negative quantity
    ...
    test('Property 2 Extension: Atomic transaction behavior', () async {
      ...
      final initialTransactionCount = await TransactionPayment.db.count(
        session,
      );
      final initialConsumableCount = await TransactionConsumable.db.count(
        session,
      );
      ...
      final finalConsumableCount = await TransactionConsumable.db.count(
        session,
      );
    ...
    test(
      ...
      () async {
        ...
        for (int i = 0; i < 5; i++) {
          ...
          for (final entry in items.entries) {
            ...
            final inventoryRecord = finalInventory.firstWhere(
              ...
              orElse: () => throw StateError(
                'Inventory record not found for $consumableType',
              ),
            );
            ...
            final timeDiff = DateTime.now()
                .difference(inventoryRecord.lastUpdated)
                .inSeconds
                .abs();
            expect(timeDiff, lessThan(5)); // Within 5 seconds
          }
          ...
          final preSecondFulfillInventory = await InventoryManager.getInventory(
            session,
            accountId,
          );
          ...
          final postSecondFulfillInventory =
