  ...
  group('WebhookHandler Unit Tests', () {
    ...
    group('Webhook Routing to Correct Rails', () {
      ...
      test('should route webhooks to different rails correctly', () async {
        ...
        final x402Result = await x402Rail.processCallback({'orderId': 'x402_order'});
        final moneroResult = await moneroRail.processCallback({'orderId': 'monero_order'});
    ...
    group('Transaction Status Updates', () {
      test('should process successful webhook results correctly', () async {
        ...
        final mockRail = MockWebhookRail(PaymentRail.x402_http, shouldSucceed: true);
        PaymentManager.registerRail(mockRail);
      ...
      test('should handle webhook with no orderId gracefully', () async {
        ...
        final mockRail = MockWebhookRail(PaymentRail.x402_http, includeOrderId: false);
        PaymentManager.registerRail(mockRail);
        
        final webhookData = {
          'success': true,
          'transactionHash': 'tx_hash_no_order',
        };
      ...
      test('should handle failed payment webhooks correctly', () async {
        ...
        final mockRail = MockWebhookRail(PaymentRail.x402_http, shouldSucceed: false);
        PaymentManager.registerRail(mockRail);
    ...
    group('Error Handling for Invalid Webhooks', () {
      ...
      test('should handle PaymentException from rails', () async {
        ...
        final paymentExceptionRail = PaymentExceptionThrowingWebhookRail(PaymentRail.monero);
    ...
    group('Webhook Utility Methods', () {
      ...
      test('should extract order ID from various webhook formats', () {
        ...
        expect(WebhookHandler.extractOrderId({'orderId': 'order123'}), equals('order123'));
        expect(WebhookHandler.extractOrderId({'order_id': 'order456'}), equals('order456'));
        expect(WebhookHandler.extractOrderId({'externalId': 'order789'}), equals('order789'));
        expect(WebhookHandler.extractOrderId({'external_id': 'order000'}), equals('order000'));
        
        // Test precedence (orderId should take priority)
        expect(WebhookHandler.extractOrderId({
...
class MockWebhookRail implements PaymentRailInterface {
  ...
  Future<PaymentResult> processCallback(Map<String, dynamic> callbackData) async {
    ...
    return PaymentResult(
      ...
      transactionHash: shouldSucceed ? 'mock_tx_hash_${DateTime.now().millisecondsSinceEpoch}' : null,
