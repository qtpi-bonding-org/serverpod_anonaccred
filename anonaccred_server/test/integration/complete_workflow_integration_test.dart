...
void main() {
  withServerpod('Complete Workflow Integration Tests', (sessionBuilder, endpoints) {
    ...
    group('Account Creation → Device Registration → Authentication Flow', () {
      test('complete happy path workflow with real Ed25519 signatures', () async {
        ...
        final signature = await algorithm.sign(challengeBytes, keyPair: deviceKeyPair);
        ...
        final devices = await endpoints.device.listDevices(sessionBuilder, account.id!);
      ...
      test('authentication fails with wrong signature', () async {
        ...
        final challengeBytes = utf8.encode(challenge);
        final wrongSignature = await algorithm.sign(challengeBytes, keyPair: wrongKeyPair);
    ...
    group('Device Revocation → Authentication Failure Flow', () {
      test('revoked device cannot authenticate', () async {
        ...
        final initialChallenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
        final initialChallengeBytes = utf8.encode(initialChallenge);
        final initialSignature = await algorithm.sign(initialChallengeBytes, keyPair: deviceKeyPair);
        ...
        final postRevocationChallenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
        final postRevocationChallengeBytes = utf8.encode(postRevocationChallenge);
        ...
        final postRevocationAuthResult = await endpoints.device.authenticateDevice(
          sessionBuilder,
          devicePublicKeyHex,
      ...
      test('revocation immediately blocks all authentication attempts', () async {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          expect(authResult.success, isFalse, 
                 reason: 'Authentication attempt ${i + 1} should fail for revoked device');
          expect(authResult.errorCode, isNotNull);
        }
      });
    });
    ...
    group('Multi-Device Scenarios with Different Keys', () {
      test('multiple devices can authenticate independently', () async {
        ...
        final deviceList = await endpoints.device.listDevices(sessionBuilder, account.id!);
      ...
      test('revoking one device does not affect others', () async {
        ...
        final deviceToRevoke = devices[1]['device'];
        await endpoints.device.revokeDevice(
          sessionBuilder,
          account.id!,
          deviceToRevoke.id!,
        ...
        for (final deviceIndex in [0, 2]) {
          ...
          expect(authResult.success, isTrue, 
                 reason: 'Device $deviceIndex should still authenticate after device 1 revocation');
        }
        
        // Verify revoked device (index 1) cannot authenticate
        final revokedDeviceData = devices[1];
        final revokedChallenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
        ...
        expect(revokedAuthResult.success, isFalse, 
        ...
        final revokedDevice = deviceList.firstWhere((d) => d.id == deviceToRevoke.id);
      ...
      test('cross-device authentication fails (device A cannot use device B signature)', () async {
        ...
        final challenge = await endpoints.device.generateAuthChallenge(sessionBuilder);
        final challengeBytes = utf8.encode(challenge);
        final deviceBSignature = await algorithm.sign(challengeBytes, keyPair: deviceBKeyPair);
        ...
        expect(crossAuthResult.success, isFalse, 
