...
void main() {
  group('Module Packaging Integration Tests', () {
    test('module export provides all public API components', () {
      ...
      expect(() => AnonAccredExceptionFactory.createException(
        code: AnonAccredErrorCodes.internalError,
        message: 'Test',
      ), returnsNormally);
      
      // Error classification should be available
      expect(() => AnonAccredExceptionUtils.isRetryable(
        AnonAccredErrorCodes.internalError
      ), returnsNormally);
      
      // Crypto utilities should be available
      expect(() => CryptoUtils.isValidEd25519PublicKey('invalid'), 
        returnsNormally);
      
      // Privacy logger should be available (class exists)
      expect(PrivacyLogger, isNotNull);
    });
    ...
    test('exception factory creates consistent exception structures', () {
      ...
      expect(authException.code, equals(AnonAccredErrorCodes.authInvalidSignature));
      ...
      expect(paymentException.code, equals(AnonAccredErrorCodes.paymentInsufficientFunds));
      ...
      expect(inventoryException.code, equals(AnonAccredErrorCodes.inventoryInsufficientBalance));
    ...
    test('error classification provides comprehensive analysis', () {
      ...
      expect(analysis, containsPair('code', AnonAccredErrorCodes.authInvalidSignature));
      ...
      expect(analysis['retryable'], isFalse); // Invalid signature is not retryable
