...
void main() {
  group('CryptoAuth Integration Tests', () {
    test('verifies real Ed25519 signatures correctly', () async {
      ...
      final isValid = await CryptoAuth.verifySignature(publicKeyHex, messageUint8List, signatureHex);
      expect(isValid, isTrue, reason: 'Real Ed25519 signature should verify correctly');
      
      // Test CryptoAuth.verifyMessageSignature with real signature
      final isValidMessage = await CryptoAuth.verifyMessageSignature(publicKeyHex, message, signatureHex);
      expect(isValidMessage, isTrue, reason: 'Real Ed25519 message signature should verify correctly');
      ...
      final challengeSignature = await algorithm.sign(challengeBytes, keyPair: keyPair);
      final challengeSignatureHex = challengeSignature.bytes.map((b) => b.toRadixString(16).padLeft(2, '0')).join();
      
      final result = await CryptoAuth.verifyChallengeResponse(publicKeyHex, challenge, challengeSignatureHex);
      expect(result.success, isTrue, reason: 'Real Ed25519 challenge response should succeed');
    ...
    test('rejects invalid signatures correctly', () async {
      ...
      final result = await CryptoAuth.verifyChallengeResponse(publicKeyHex, challenge, invalidSignature);
      expect(result.success, isFalse, reason: 'Invalid signature should be rejected');
      expect(result.errorCode, equals(AnonAccredErrorCodes.authInvalidSignature));
    });
    ...
    test('challenge generation produces unique values', () {
      ...
      for (int i = 0; i < 10; i++) {
        final challenge = CryptoAuth.generateChallenge();
        expect(challenge.length, equals(64), reason: 'Challenge should be 64 hex characters');
        expect(challenges.contains(challenge), isFalse, reason: 'Challenges should be unique');
